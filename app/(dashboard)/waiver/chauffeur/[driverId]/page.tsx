...
import { BankType, DistrictType, StateType, TransmissionType, UserDetails, VehicleType, WorkExperience } from "@/lib/types";
import { getBanks, getDistricts, getLocations, getStates, getTransmissionTypes, getVehicleTypes, getWorkExperiences } from "@/actions/ride-cost";
...
export default async function DetailsPage({ params }: Props) {
  ...
  let user;
  if (edit) {
    user = await getUserDetails(params.driverId, "CHR");
    console.log(user);
  }
  const heading = edit ? `Details of ${user.fullname}` : "Add New Chauffeur";
  ...
  const experiences: WorkExperience[] = await getWorkExperiences();
