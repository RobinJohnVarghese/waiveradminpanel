...
import { BankType, DistrictType, StateType, TransmissionType, VehicleType, WorkExperience, } from "@/lib/types";
...
export const EditDetails = ({
  ...
  const promptSchema = z.object({
    ...
    driving_experience: z.string(),
    transmission_type: z.any().optional(),
    vehicle_types: z.any().optional(),
    license_validity: z.string(),
    address: z.string(),
  ...
  async function onSubmit(data: FormData) {
    ...
    if (initialData && initialData.id) {
      await axios
        .put(
          `${process.env.BACKEND_URL}/api/v1/staff/chauffeur-profile/`,
          formData,
          {
            headers: {
        ...
        .then((res) => {
          ...
          setDisabled(false);
          router.replace("/waiver/chauffeur");
          router.refresh();
        })
        .catch((error) => {
  ...
  const [selectedTransmissionType, setSelectedTransmissionType] = useState<VehicleType[]>(initialData?.transmission_type ? initialData?.transmission_type : []);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          /> */}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Experience in driving (Years)</FormLabel>
                  <Select
                    onValueChange={field.onChange}
                    defaultValue={field.value}
                  >
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue placeholder="Select experience in driving" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      {experiences &&
